                ...
                return Column(
                  children: [
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          const Icon(Icons.lock_rounded, color: Color(0xFFFF9900), size: 18),
                          const SizedBox(width: 8),
                          Expanded(
                            child: Text(
                              '${file.name} (Solo Lectura)',
                              style: GoogleFonts.firaCode(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.white),
                            ),
                          ),
                          IconButton(
                    ...
                    Container(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: () {
                          ...
                          final prompt = 'Por favor edita el archivo "${file.path}" para: ';
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('Comando de edición enviado a Claude...', style: GoogleFonts.outfit()),
                        ...
                        icon: const Icon(Icons.auto_fix_high_rounded, color: AppTheme.accentBlue, size: 16),
                        label: Text('PEDIR CAMBIO A CLAUDE', style: GoogleFonts.outfit(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white)),
                      ),
                    ),
                          ...
                          : Container(
                              ...
                              child: SingleChildScrollView(
                                ...
                                child: SelectableText(
                                  ...
                                  style: GoogleFonts.firaCode(fontSize: 12, color: const Color(0xFF00FF66), height: 1.4),
                                ),
                              ),
                            ),
                    ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Explorador Solo Lectura', style: GoogleFonts.outfit(fontWeight: FontWeight.bold, fontSize: 18)),
        actions: [
          IconButton(
          ...
          IconButton(
            icon: const Icon(Icons.arrow_upward_rounded, color: AppTheme.accentBlue),
            tooltip: 'Subir nivel',
          ...
          IconButton(
            ...
            tooltip: 'Recargar',
            onPressed: () {
              final cur = context.read<ReadonlyExplorerCubit>().state.currentPath;
          ...
          return Column(
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.folder_open_rounded, color: AppTheme.accentBlue, size: 18),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        state.currentPath,
                        style: GoogleFonts.firaCode(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white70),
              ...
              if (state.status == ReadonlyExplorerStatus.loading)
                ...
                Expanded(
                  child: state.files.isEmpty
                      ? Center(child: Text('Directorio vacío.', style: GoogleFonts.outfit(color: AppTheme.textSecondary)))
                      : ListView.builder(
                          ...
                          itemBuilder: (context, idx) {
                            ...
                            return Card(
                              ...
                              shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(8),
                                side: BorderSide(
                                  color: isEnv ? const Color(0xFF00FF66).withOpacity(0.4) : AppTheme.border.withOpacity(0.4),
                                ),
                              ),
                              child: ListTile(
                                dense: true,
                                leading: Icon(
                                  item.isDir
                                      ...
                                      : (isEnv ? Icons.key_rounded : Icons.insert_drive_file_rounded),
                                  color: item.isDir
                                      ? AppTheme.accentBlue
                                      : (isEnv ? const Color(0xFF00FF66) : AppTheme.textSecondary),
                                  size: 22,
                                ),
                                title: Text(
                                  item.name,
                                  style: GoogleFonts.firaCode(
                                    fontSize: 13,
                                    fontWeight: item.isDir || isEnv ? FontWeight.bold : FontWeight.normal,
                                    color: isEnv ? const Color(0xFF00FF66) : Colors.white,
                                  ),
                                ),
                                subtitle: item.modifiedAt != null
                                    ? Text(item.modifiedAt!, style: GoogleFonts.firaCode(fontSize: 10, color: AppTheme.textSecondary))
                                    : null,
                                trailing: item.isDir ? const Icon(Icons.chevron_right_rounded, color: AppTheme.textSecondary, size: 18) : null,
                                onTap: () {
                                  if (item.isDir) {
                                    context.read<ReadonlyExplorerCubit>().listDirectory(item.path);
                                  } else {
