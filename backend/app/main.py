...
from backend.app.routers import auth, tickets, claude, infra, backups, docs, explorer, registro, documentacion, system, workflow
...
# workflow.router va ANTES que tickets.router a proposito: tickets.router
# define un catch-all /tickets/{code} que, si se registra primero, intercepta
# /tickets/lifecycle tratando "lifecycle" como si fuera un codigo de ticket
# (paso el 2026-08-19, devolvia 404 "Ticket lifecycle no encontrado"). El
# orden entre routers importa igual que el orden DENTRO de un mismo router.
app.include_router(workflow.router, prefix=settings.API_V1_STR, dependencies=protegido)
...
# /flujo queda fuera de `protegido`, igual que `auth`: es una pagina de
# referencia sin datos de negocio, pensada para abrirse en el navegador sin
# token. Va antes del mount de la web (ver comentario mas abajo) para que
# Starlette la resuelva antes de caer en el catch-all de Flutter.
app.include_router(workflow.public_router)
